...
sentToken l =  addToList (words l) "" -- words splits sentence into list of words



-- takes list of words and checks if any word contains ['.','!','?'] if yes then add to accumlator then cons with new sentences
-- if no then simply keep adding to accumlator until we find end of sentence
addToList [] x = []
addToList (x:xs) y = if (contains x ['.','!','?']) then (y++ " " ++ x):addToList xs ""
				   else addToList xs (y++ " " ++x)

contains [] _ = False
contains (x:xs) w = if (containsIn (x) w) then True
					else contains xs w

containsIn _ [] = False
containsIn y (x:xs) = if (y==x) then True
					 else containsIn y xs